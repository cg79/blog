...
func (store *ElementStore) GetElement(writer http.ResponseWriter, element string) {
    ...
    if val, ok := store.elements[element]; ok {
        fmt.Fprint(writer, strconv.Itoa(val))
    }
...
func main() {
    ...
    for scanner.Scan() {
        ...
        if parts[0] == "GET" {
        	fmt.Println(RetrieveResponse(func() (*http.Request, error) { return http.NewRequest(http.MethodGet, "http://localhost:7777/elements?id=" + parts[1], nil) }))
        } else if parts[0] == "DELETE" {
    		fmt.Println(RetrieveResponse(func() (*http.Request, error) { return http.NewRequest(http.MethodDelete, "http://localhost:7777/elements?id=" + parts[1], nil) }))    	
        }
    }
